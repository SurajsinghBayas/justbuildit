...
import { X, Send, Loader2, Sparkles, Bot, User, MessageSquare } from 'lucide-react';
...
export default function AITaskChat({ task, open, onClose }: Props) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errMsg = err?.response?.status === 501
        ...
        : err?.response?.data?.detail || 'Connection failed. Is the backend running?';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end p-4 pointer-events-none">
      ...
      {/* Chat panel */}
      <div className="relative pointer-events-auto w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden"
           style={{ height: '520px' }}>
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {messages.map((m) => (
            <div key={m.ts} className={`flex gap-2 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                m.role === 'user' ? 'bg-gray-900' : 'bg-gradient-to-br from-violet-500 to-purple-600'
              }`}>
                {m.role === 'user'
        ...
        <div className="px-4 pb-4 pt-2 border-t border-gray-50 flex-shrink-0">
          ...
          <p className="text-[10px] text-gray-300 mt-1.5 text-center">Powered by AWS Bedrock</p>
        </div>
      </div>
    </div>
  );
